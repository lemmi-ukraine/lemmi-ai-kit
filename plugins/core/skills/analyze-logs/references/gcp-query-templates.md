# Log Query Templates — GCP Cloud Logging Worked Example

**A worked example in one query dialect, not the required backend.** SKILL.md's *Querying the Log
Platform Directly* states the method — narrowest resource filter, explicit time bounds, the right
field for the target, validate against a downloaded sample first. This file works that method
through in GCP Cloud Logging's syntax. Every query below translates: the filter clauses map to
CloudWatch Logs Insights `filter`, to a LogQL stream selector plus line filter, or to an Elasticsearch
`bool`/`must` query. What does not translate is the *shape* of the questions, and that is the part
worth copying.

Replace `{SERVICE_NAME}` with the service name and `{PROJECT_ID}` with the project id of the
environment you are querying.
...
**The most transferable entry in this file, and the least GCP-specific.** The lesson is that a job
system often has *no single* structured failure event, so a query written against the first failure
shape you find reports a clean system. In the example system below, a job's terminal outcome takes
one of FOUR shapes — verified against the job service's source AND two real incidents' logs, because
the fourth shape was missed until a second incident produced it. Enumerate the shapes in *your*
system before trusting any failure count:
...
`jsonPayload.job_id` — with a **JSON parse, not `grep -A`/`-B`**, because console log exports do not
guarantee that related entries are adjacent, so a proximity-based grep silently drops the entries
that matter. Treat "created/dispatched/execution_started with no completed/expired and no Exception
message" as inconclusive rather than failed: it may simply be still running, or outside the captured
window.
...
Tips 1, 4, 5, 6 and 7 hold on every platform; 2 and 3 are the GCP spelling of "structured payload
field vs. raw text field".

1. **Always include a time range** — an unbounded scan is slow, and on a hosted platform it is billable
2. **Use `jsonPayload.*` for structured fields** — for logs emitted as structured JSON
...
The GCP column of SKILL.md Step 1a's role table, expanded.
