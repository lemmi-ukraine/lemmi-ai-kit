...
|---|---|---|---|---|---|
| PF-01 | main | please note that it's worth noting; as you can see, this probably maybe may might possibly apparently seems and looks like it appears to be likely and presumably wrong; I think it is simply just obviously clearly invalid in order to additionally and furthermore exercise should | `IdlePolicy.evaluate` → `IdlePolicy.is_eligible` → `IdlePolicy._reset` | PASS | `code: src/policy.py resets state when eligibility fails` |
