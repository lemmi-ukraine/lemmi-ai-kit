...
> `conformant.md` except the Coverage note for the primary cited file is
> `_none_` instead of a real read-status note. Drives the SS6 refusal: the
> generator must exit 2 and list every offending file, never emit Coverage.
...
|---|---|---|
| `src/policy.py` | FP-01, FP-02, FP-03 | _none_ |
