...
> **GENERATOR FIXTURE — not runtime documentation.** Byte-identical to
> `conformant.md` except one deliberately wrong derived cell: the Symbol index
> `scenarios` cell for `IdlePolicy.is_eligible` is missing FP-03,
> which its path actually carries. Drives the positive half of the probe pair
> (`--certify` must report exactly 1 differing row).
...
| `IdlePolicy.is_eligible` | `src/policy.py` | FP-01, FP-02 | `IdlePolicy.evaluate` | — |
