...
# Initiative Cleanup — settle, preserve, retire, trim — [pack prerequisites](../../README.md)
...
Run the artifact-kind gate before dispositioning specs, briefs, instruments, captures, or results:

```bash
python "$S" kinds --root .specs/<initiative> --list
```

Each file gets exactly one kind and a life-ending condition. Kind is not disposition: pending
instruments, rollback anchors, and result corpora stay even when they look like scratch. Use
[references/artifact-kinds.md](references/artifact-kinds.md) for the full taxonomy and the required
`extraction` / `fundep` sub-gates.
...
Use the eight questions in [references/self-review-gate.md](references/self-review-gate.md). They
cover per-file disposition, refs/fundep gates, code-backed evidence, branch sequencing, unavailable
facts, kind coverage, life-ending conditions, and executable dependencies.
...
- **Ask the operator to run `/lemmi-ai-kit-core:learning-consolidator`** — do **not** invoke it. It is a workflow, and
...
The remaining anti-patterns are judgment checks whose details live in the references. Before any
cleanup plan is presented, review the relevant reference instead of relying on memory:

- [references/artifact-kinds.md](references/artifact-kinds.md) for kind vs disposition mistakes.
- [references/tmp-classification.md](references/tmp-classification.md) for `.ai/tmp/` scratch vs input handling.
- [references/comment-pass.md](references/comment-pass.md) for scoped comment trimming.
- [references/self-review-gate.md](references/self-review-gate.md) for the measured cleanup failure cases.
