...
# Initiative Planner — decide the shape of the whole initiative before any of it runs — [pack prerequisites](../../README.md)
...
| Written by | **`/lemmi-ai-kit-core:product-brief`** — it owns this shape end to end | **this step**, from the template |
...
**Both shapes carry the same four load-bearing fields**, and `/lemmi-ai-kit-core:product-brief`'s output does **not**
include three of them — its template runs Problem · Behavior · UX Content · Out of Scope ·
Implementation Notes. So on the PDR path, run `/lemmi-ai-kit-core:product-brief` first, then **add the charter block**
from the template:
...
The bar for either shape is `/lemmi-ai-kit-core:product-brief`'s **Dimension 1 (Problem Strength)**: *falsifiable,
...
**When a session must work a branch it does not have checked out, the brief names `/lemmi-ai-kit-core:branch-switch`
