...
# Stacked PR Planner — decide the topology before the first commit — [pack prerequisites](../../README.md)
...
- **Branch hygiene:** never `git symbolic-ref` to switch branches — it moves HEAD without touching
  the working tree or index and leaves all three inconsistent. Use `/lemmi-ai-kit-core:branch-switch`. Run
...
against its own initiative (`/lemmi-ai-kit-skill-authoring:skill-creation-workflow` Phase 8, on a live stacked base pinned by
