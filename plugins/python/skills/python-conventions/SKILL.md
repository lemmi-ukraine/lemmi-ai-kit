...
### Aliased Pydantic Model Construction

```python
# Model with Field(alias=...) + populate_by_name (Core API camelCase contract):
class FeatureAvailability(BaseModel):
    feature_type: int = Field(alias="featureType")
    model_config = {"populate_by_name": True}

# GOOD: construct with the ALIAS kwargs — basedpyright synthesizes __init__
# from aliases, not field names
FeatureAvailability(featureType=11)

# BAD: valid at runtime (populate_by_name), but basedpyright reports an unknown argument
FeatureAvailability(feature_type=11)
```

### Partial DI Test Doubles — Double-Cast

```python
# House idiom: a minimal fake that implements only part of the target's surface
validator = cast(ResponseValidator, cast(object, fake_validator))
```

Both hops are always-valid casts, so the idiom needs no `# type: ignore` and stays clean
under any diagnostic config — basedpyright's `reportInvalidCast` is off under `strict`
but active in `recommended`/`all`, so an idiom that needs no cast suppression survives a
tightened config unchanged.

### `asyncio.gather(return_exceptions=True)` Result Partitioning

```python
results = await asyncio.gather(*tasks, return_exceptions=True)  # list[T | BaseException]
for result in results:
    if isinstance(result, QuestionAnalysis):  # narrow on the success type
        successful.append(result)
    elif isinstance(result, Exception):       # narrow failures on Exception
        log_exception(self.logger, result, ...)
```

Partition positively — once on the success type, once on `Exception`: the positive checks
type both branches, and the failure payload is always an `Exception` in practice because
BaseException-only exceptions (KeyboardInterrupt, CancelledError) propagate out of `gather`
...
surface. If such a wrapper must satisfy a `Protocol` at a boundary before its explicit methods
exist, a single `cast(ProtocolName, wrapper)` at the one wiring site is the accepted stopgap —
never a cast per call site, and never a reason to skip the explicit methods.
...
### `Request` Parameters on Depends Targets

Never annotate a `Depends` target's parameter as `Request | None` — FastAPI treats only a
bare `Request` annotation as the special request injection; with a union it tries to model
the parameter as a body/query field and the app fails at startup. Keep `Request`, and at
manual call sites that have no HTTP request (WebSocket flows) pass `request=cast(Any, None)`.
Plain helper functions that are never `Depends` targets may take `Request | None` freely.
...
### Lazy Settings Proxies

A config module that exports a lazy proxy under a union annotation
(`settings: Settings | _LazySettingsProxy`) can be imported without requiring env vars
at import time. When a consumer needs the concrete typed surface, declare ONE
module-level alias — `_settings = cast(Settings, settings)` — never `cast(Any, …)` at
each call site.
