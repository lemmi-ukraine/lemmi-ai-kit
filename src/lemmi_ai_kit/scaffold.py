...
def _skill_line(entry: SkillEntry) -> str:
    if entry.invocation == "user":
        return f"- `/{_PLUGIN_NAME}:{entry.name}` — {entry.summary}"
...
def render_claude_md(manifest: Manifest) -> str:
    ...
    The plugin ships every skill, so the index always lists the whole catalog,
    namespaced the way plugin skills are invoked (`/lemmi-ai-kit:<name>`).
    """
